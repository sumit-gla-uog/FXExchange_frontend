...
export const AdminRatesPage = () => {
  ...
  const { dashData, dashLoading, rates, ratesLoading, pairs, handleManualUpdate, handleCsvUpload, refreshRates } = useAdminRates()
  const { control, handleSubmit, reset, formState: { isSubmitting } } = useForm<ManualRateForm>({ defaultValues: { pair_id: "", rate: "" } })

  const onManualUpdate = async (values: ManualRateForm) => {
    setManualError(""); setManualSuccess("")
    try {
      await handleManualUpdate(values)
  ...
  const onCsvUpload = async (file: File) => {
    ...
    try {
      const result = await handleCsvUpload(file, BASE_URL)
      setCsvResult(result)
    ...
    <StackLayout gap={3}>
      ...
      <Card className="rates-card">
        ...
        <div className="rates-method-cards">
          <Card className="rates-method-card" onClick={() => setActiveMethod(activeMethod === "csv" ? null : "csv")} style={{ border: `1px solid ${activeMethod === "csv" ? "#0f766e" : "#e5e7eb"}`, background: activeMethod === "csv" ? "#f0fdf9" : "white" }}>
            <FlexLayout align="center" gap={2}>
              <div className="rates-method-card__icon rates-method-card__icon--csv"><UploadIcon size={2} /></div>
              <StackLayout gap={0}>
        ...
        {activeMethod === "csv" && (
          <StackLayout gap={2}>
            ...
            <CSVDropZone onUpload={onCsvUpload} />
        ...
        {activeMethod === "manual" && (
          <form onSubmit={handleSubmit(onManualUpdate)}>
      ...
      <Card className="rates-card">
        <FlexLayout justify="space-between" align="center" style={{ marginBottom: 16 }}>
          ...
          <Button appearance="transparent" onClick={refreshRates} style={{ color: "#0f766e" }}><RefreshIcon size={1} /></Button>
        </FlexLayout>
        {ratesLoading ? (
